...
<div class="max-w-7xl mx-auto px-6 py-10">
    ...
    {{-- HEADER --}}
    <div class="mb-12">
        <h1 class="text-4xl font-bold text-white">Nieuwe offerte</h1>
    ...
    <form method="POST" action="{{ route('quotes.store') }}" class="bg-gradient-to-br from-slate-800/90 to-slate-900 border border-slate-700 rounded-2xl p-8 shadow-xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="valid_until" class="block mb-2 text-sm font-semibold text-slate-300">Geldig tot</label>
                <input type="date" id="valid_until" name="valid_until" class="w-full px-4 py-3 rounded-lg bg-slate-700/50 border border-slate-600 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent" value="{{ old('valid_until') }}">
            </div>

            <div>
                <label for="preferences" class="block mb-2 text-sm font-semibold text-slate-300">Voorkeuren</label>
                <input type="text" id="preferences" name="preferences" class="w-full px-4 py-3 rounded-lg bg-slate-700/50 border border-slate-600 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent" value="{{ old('preferences') }}" placeholder="Bijv. levering binnen 2 weken">
            </div>
        </div>
        ...
        <div class="space-y-4" id="lines-wrapper">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 line-item">
                <div>
                    ...
                    <select name="items[0][product_id]" class="w-full px-4 py-3 rounded-lg bg-slate-700/50 border border-slate-600 text-white product-select focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="number" min="1" name="items[0][quantity]" class="w-full px-4 py-3 rounded-lg bg-slate-700/50 border border-slate-600 text-white quantity-input focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent" value="1" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="items[0][unit_price]"
                           class="w-full px-4 py-3 rounded-lg bg-slate-700/50 border border-slate-600 text-white unit-price focus:outline-none"
                           readonly>
                </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-4">
            <a href="{{ route('quotes.overview') }}" class="px-6 py-3 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white transition font-medium"></a>
                Annuleren
            </a>
