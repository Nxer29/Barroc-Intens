...
<div class="w-full max-w-6xl mx-auto py-8">
    ...
    <div class="bg-gray-900 rounded-2xl border border-yellow-400/40 p-6 shadow-lg mb-6">
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-3xl font-bold text-yellow-400 mb-1">{{ $customer->displayName() }}</h1>
...
<a href="{{ route('maintenance.requests.create', $customer) }}"
    ...
    {{-- DETAILS --}}
    <div class="bg-gray-900 rounded-2xl border border-gray-800 shadow-lg p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-300">
            <div>
                <strong>Contactpersoon</strong>
                <div class="text-gray-200">{{ $customer->contact_name ?? '-' }}</div>
            </div>

            <div>
                <strong>E-mail</strong>
                <div class="text-gray-200">{{ $customer->contact_email ?? '-' }}</div>
            </div>

            <div>
                <strong>Telefoon</strong>
                <div class="text-gray-200">{{ $customer->contact_phone ?? '-' }}</div>
            </div>

            <div>
                <strong>Status</strong>
                <div class="text-gray-200">{{ $customer->status ?? '-' }}</div>
            </div>
        </div>
    </div>

    {{-- TABS --}}
    <div class="mt-6 bg-gray-900 rounded-2xl border border-gray-800 shadow-lg">
